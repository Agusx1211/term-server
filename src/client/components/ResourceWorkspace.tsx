...
import { FileCode2, Image, LoaderCircle, Save } from "lucide-preact";
      ...
      {tabs.map((tab) => (
        <div key={tab.path} class={`resource-document ${activePath === tab.path ? "active" : "cached"}`}>
          {tab.type === "image" ? (
            ...
            <TextDocument tab={tab} theme={theme} onDirty={onDirty} onNotice={onNotice} />
          )}
        </div>
      ))}
    </div>
...
function TextDocument({ tab, theme, onDirty, onNotice }: TextDocumentProps) {
  ...
  return (
    <section class="text-document">
      <header class="resource-document-header">
        ...
        <span>{tab.path}</span>
        <em>{language}</em>
        <button class="resource-save" onClick={() => void save()} disabled={loading || saving || !tab.dirty}>
