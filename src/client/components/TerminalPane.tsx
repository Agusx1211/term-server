  ...
  useEffect(() => {
    ...
    const connect = () => {
          ...
          try {
            ...
            if (binary instanceof ArrayBuffer) {
              ...
              pendingWrites = writeTerminal(frame.data, commit, generation, next)
                ...
                .catch(failProtocol)
                .finally(() => {
                  if (isCurrentSocket(generation, next)) backlog.settle(settled);
                });
              return;
            }
            ...
            if (isCurrentSocket(generation, next)) backlog.settle(queuedBytes);
          }
        }).catch((error) => {
          failProtocol(error);
        });
      ...
      next.addEventListener("close", () => {
        ...
        attempts += 1;
        void settledStream(generation).then(() => {
          if (!isCurrentGeneration(generation) || exited.current) return;
          backlog.reset();
          if (recoveringOutput) stream.restart();
          clearReconnectTimer();
