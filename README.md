...
The broker makes both virtual devices the host defaults while it runs and restores the previous defaults on a clean shutdown. Browser audio is accepted only on the authenticated, origin-checked WebSocket and is never written to disk or included in debug recordings. Programs using the virtual microphone decide whether audio stays local or goes to another service. Browser output-device selection uses `setSinkId`; browsers without it can still use their system default speaker.
