...
### Virtual microphone and speaker

On Linux, the session broker creates **Term Server Microphone** and **Term Server Speaker** through the current user's PulseAudio server or PipeWire PulseAudio compatibility service after a browser enables either audio route. The host needs `pactl`, `parec`, and the `module-pipe-source` and `module-pipe-sink` modules. Debian and Ubuntu desktop installations normally already have the modules; `pulseaudio-utils` provides both commands, while a minimal host also needs either PulseAudio or PipeWire's PulseAudio service. If the audio service, commands, or modules are missing, the Audio panel stays off and reports the exact setup failure without affecting terminals.

Open **Audio** in the desktop status bar or mobile toolbar. Choose a browser microphone to mix it into the host virtual microphone, choose a browser speaker to hear the host virtual speaker, or leave either selector **Off**. Several signed-in tabs can select inputs and outputs at once; microphone streams are averaged before they enter the shared host device, and one host speaker stream fans out to every listening tab. A 60 ms prebuffer on each leg absorbs normal scheduling and network jitter, while stale audio is dropped instead of growing latency.

The broker changes the host defaults only after an audio route is enabled, then keeps the virtual devices ready until broker shutdown and restores the previous defaults. On startup it removes virtual devices left by a crashed broker and repairs defaults that still point to them. Browser audio is accepted only on the authenticated, origin-checked WebSocket and is never written to disk or included in debug recordings. Programs using the virtual microphone decide whether audio stays local or goes to another service. Browser output-device selection uses `setSinkId`; browsers without it can still use their system default speaker.
