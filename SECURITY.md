...
Fili-generated titles and notification summaries are independently enabled by default but stay idle until a model is selected; no request leaves the machine until then. Enabling titles sends a bounded copy of the submitted task message to the selected model provider. Enabling notification summaries sends a bounded, ANSI-sanitized tail of terminal output, which may contain source code, command output, paths, or secrets. Use only a provider appropriate for that data. Fili is an in-process labelling agent, not a shell: it has no project context, skills, or built-in tools, and exposes only terminal reads (`list_terminals`, `read_terminal`, `read_transcript`) plus a single labelling tool (`set_label`) that can only rename automatic-named tabs and write completion summaries, but model-provider data handling remains governed by the selected provider.
